...
- claim（`claim_task`）: 候補を`task_row`で読み、`task::claim`で`in_progress`にし、`Uuid::new_v4`のrun IDと`now`の時刻で`TaskRun::new`を作る。taskは`UPDATE tasks SET status=<集約のstatus> ... WHERE status='ready'`（条件は並行の変更の検出）で、runは集約の値（status、provider、小文字の`base_commit`、`created_at`を明示）で`INSERT`する。integrateの`completed`化はまだSQLにある（integrateのタスクで扱う）。
