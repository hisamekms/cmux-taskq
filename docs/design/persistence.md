...
- 更新: 行を読んで復元し、domainのコマンド（`task::transition`、`task::set_goal`、`task::set_paths`、`goal::edit`、`goal::ready`、`goal::close`）に渡して、返った集約の値を`UPDATE`する。`updated_at`は`Clock::timestamp()`をbindした値（`close_goal`だけは`goal::close`に渡した`closed_at`と同じ値）で、書いた後に行を読み直して返す。`transition_task`の`WHERE status=<読んだstatus>`と`close_goal`の`WHERE closed_at IS NULL`は並行の変更を見つけるための条件で（`BEGIN IMMEDIATE`の中なので通常は起きない）、どの遷移を許すかの判断はdomainにある。`goal_updated`の`old`は`goal::edit`が集約を消費する前にJSONにしておく。
