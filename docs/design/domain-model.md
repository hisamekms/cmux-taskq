...
- `Proposal`: plannerがplan reviewに出すgoal（0個以上）とtaskの束に、持ち主のplanner（`PlannerOwner`: 人が開いたか（`person`）runtimeが立てたか（`runtime`）の`PlannerOrigin`と、cmux workspaceのUUID）を結び付けたもの（[ADR-0041](../adr/0041-on-demand-planners-proposals-submitted-and-plan-review-job.md)の決定7）。`status`（`ProposalStatus`: `submitted`（plan review待ち） | `revising`（plannerに差し戻し中） | `accepted`（passしてtaskがreadyになった） | `canceled`（人のanswerでcancel））、最後にsubmitした時刻`submitted_at`、差し戻しの回数`revise_count`を持つ。taskとgoalは同時に1つのactive（`submitted` / `revising`）なproposalにだけ属する（`proposal::check_task_joins` / `check_goal_joins`。`accepted` / `canceled`のproposalはmemberを縛らない）。集約は`src/domain/proposal.rs`。
...
- task lease: runを持たないtask（follow-up triageの途中のdraft）を所有するsupervisorのtokenとheartbeat（表`task_leases`、ADR-0037の決定3）。domainの型にはせず、storeが`HEARTBEAT_TIMEOUT_SECS`で鮮度を判定する。
...
- taskのstatusは`draft`（まだ出していない） → `submitted`（plan review待ち） → `ready` → `in_progress` → `completed`と、`canceled`（ADR-0041の決定8）。`add`でdraftを作る。遷移は`TaskAction`で決まる（`TaskStatus::transition`）:
...
- `in_progress`のTaskは、未完了run（claimed/starting/running/validating/awaiting_integration/integrating/needs_session）がある間は手動変更できない。すべてのrunが`failed`または`interrupted`になった`in_progress`は`ready`/`draft`/`canceled`へ手動で戻せる。再試行は新しいTaskRunになる。終端状態は変更できない。依存の追加・削除はdraft / submitted / readyだけに許可する（plan reviewは`submitted`のtaskに依存を足し、優先度を下げる。ADR-0041の決定11）。
...
| `follow_up.rs` | follow-up triageのverdict（`FollowUpDecision`、`FollowUpVerdict`、`FollowUpProposal`と`check` / `new_task`）、`adopt_override`（上書き規則）、`FollowUpAction`、`FOLLOW_UP_OPTIONS`（[Follow-up triage](#follow-up-triage)） |
...
- follow_upのdraftの採否はfollow-up triageのverdictか`follow_up`のaskのanswerで決まり、適用はどちらも1トランザクションで、draftを再検査してから行う（[Follow-up triage](#follow-up-triage)）。同じdraftに2度適用されない。
...
[ADR-0037](../adr/0037-follow-up-triage-job-decides-follow-up-drafts.md)の型は`src/domain/follow_up.rs`、適用は`src/infrastructure/follow_ups.rs`（port `FollowUpStore`）にある。supervisorのループへの組み込みは後続のtaskで行う。

- **verdict**: jobのstdoutは`FollowUpVerdict`（`verdict`: `adopt | drop | ask`、`reason`、任意の`task`（`FollowUpProposal`）、`question`）。`FollowUpVerdict::parse`はreview / triageと同じく全体か最外の`{...}`を読み、未知のフィールドは（`task`の中も）拒否する。`FollowUpProposal`は`title`だけが必須で、`description` / `acceptance` / `verification_commands` / `paths` / `evidence`（名前の文字列）/ `depends_on`（task ID）/ `context`は省略できる。
- **提案の検査**: `FollowUpProposal::check`がtitleの空白、空白の検証コマンド、`validate_path_globs`を通らないglob、`tests` / `e2e` / `subagent_review`以外のevidence、正でないIDを拒否する。storeがさらに`depends_on`の各taskが存在し、canceledでもdraft自身でもなく、draftに（推移的に）依存していない（draftの依存元を新しいtaskへ付け替えると循環する）ことを確かめる。
- **上書き規則**: `adopt_override(FollowUpFacts { goal_open, depth }, proposal)`が、goalがnullか閉じている、`follow_up_depth`が`FOLLOW_UP_ASK_DEPTH`（2）以上、`acceptance`が空白、の順に理由を返す。提案が無いか不正なadoptもaskにする。理由は`follow_up_triage_finished`の`overridden`とaskのquestionに残る。
- **深さ**: `tasks.follow_up_depth`はdomainの`Task`に持たせず、storeの列として`follow_up_depth` / `set_follow_up_depth`で読み書きする。`add`は0、`integrate`の登録は元のtask+1、jobのadoptはdraftの値、人のadopt（askの`adopt`）と`ready --bypass-review`（`transition_task`の`TaskAction::BypassReview`）は0。
- **適用**（storeの1トランザクション）: `adopt`は提案を`NewTask`（`FollowUpProposal::new_task`。contextの冒頭に出自）にして同じgoalに登録し、依存を張り、draftの依存元を新しいtaskへ付け替え、`TaskAction::BypassReview`で`ready`にし（plan reviewは経ない）、深さを書き、draftをcancelし、draftと新しいtaskの両方に`follow_up_adopted`を記録する。`drop`はdraftをcancelする。`ask`は`follow_up`のask（`task_id`はdraft、runなし、`asked_by: supervisor`、有効な提案があれば`FOLLOW_UP_OPTIONS`、無ければ`cancel` / `keep_draft`）を同じトランザクションで作る。結果は`FollowUpApplied`（`FollowUpAction`: `adopted` / `dropped` / `asked` / `kept_draft` / `closed`、適用後のdraft、新しいtask、ask、`overridden`）。
- **answer**: `decide_follow_up(ask_id)`がanswerを読んで適用し、askを閉じる。`adopt`は直近の`follow_up_triage_finished`の`task`を検査し直して深さ0で登録する（goalが閉じているかnullならgoalなし）。`cancel`はcancel、`keep_draft`はdraftのまま（`follow_up_triage_finished`があるので以後の対象にならない）。draftが既に`draft`でなければaskを閉じるだけ（`closed`）。自由記述、askに無いoption、有効な提案の無い`adopt`、新鮮なleaseのあるdraftは`None`で何も書かない。
