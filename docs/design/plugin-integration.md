...
plugins/claude-dagq/
  ...
  skills/dagq-land/               review ID → subagent に review.md の path を渡して結論だけ受け取る → pass なら integrate（疑義のときだけユーザーに聞く。着地後に origin へ push し、follow_ups を draft task として登録する）、draft を ready にするかをユーザーに聞く
    reference/integrate.md          review.md の中身、integrate の再検証と検証コマンド、outcome、--next
...
- maintainerはhookの出力を起点に`dagq-maintain`の手順（attentionの報告、`watch --after <cursor>`をbackgroundで再開）へ戻る。`watch`の結果で`integrate`は呼ばず、着地は`dagq-land`のレビューが通れば行い、疑義（受け入れ条件との食い違い・指示外の変更・レビューの指摘）のときだけユーザーに聞く（[ADR-0022](../adr/0022-ask-answer-inbox-planner-and-landing-on-doubt.md)決定3）。
