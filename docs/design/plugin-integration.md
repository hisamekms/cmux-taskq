...
- mainへの着地はruntimeの`integrate ID` / `integrate --next`が行う（rebase → 再検証 → squash、[ADR-0008](../adr/0008-merge-queue-squash-landing.md)）。`dagq-land` skillは`review ID`の`review.md`をsubagentにレビューさせて結論（`pass` / `concern`と理由）だけ受け取り、`pass`なら人に聞かずにこれを呼び（`watch`の結果やイベントの副作用としては呼ばない）、`concern`なら`ask --kind approve_landing --run <run_id> --option land --option send_back --option cancel`を作って次のattentionへ進み、`ask_answered`で届いたanswerが`land`ならこれを呼ぶ。`send_back`と`cancel`は、`awaiting_integration`のrunをsessionへ戻すコマンドが無く`cancel`も未完了runのあるtaskを拒否するので、当面はanswerをrunのnote（`--kind send_back` / `cancel`）に残してaskを閉じ、そのnoteのあるrunは再びレビュー・着地しない（`reference/integrate.md`）。そのうえで`outcome`（`integrated` / `needs_session` / `failed` / `no_run_awaiting`）を読んで結果を伝える。`needs_session`のrunは`dagq-session` skillに従いmaintainerが`claude --resume <run-id>`でworktreeに開き直すセッション（workspace名はrunと同じ`[<repo>]worker#<task-id> - <task title>`、descriptionは`run <run-id> resume`、[ADR-0028](../adr/0028-workspace-titles-are-repo-and-role.md)）が解消し、解消後は着地がworktreeを消すので`/exit`で終えてから`integrate`し直す。runのworkspace名は`[<repo>]worker#<task-id> - <task title>`（[ADR-0028](../adr/0028-workspace-titles-are-repo-and-role.md)。[ADR-0018](../adr/0018-run-workspace-named-after-the-task.md)の書式を上書き）、descriptionは`dagq role=worker queue=<queue hash> run=<run-id> task=<id>`で（[ADR-0026](../adr/0026-identify-workspaces-by-uuid-env-and-queue-group.md)。queueのworkspaceは`[<repo>]`のworkspace groupにまとまる）、`failed` / `interrupted`のworkspaceはruntimeが閉じないのでmaintainerが`dagq-session`に従って`cmux workspace close`する。
