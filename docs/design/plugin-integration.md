...
plugins/claude-dagq/
  ...
  skills/dagq-session/            run の session への操作: trust / permission prompt への応答（人の判断が要れば answer_prompt の ask）、worker_question の ask への回答か decide の ask での人への転送、exit_request_timed_out の /exit、failed / interrupted の workspace close、runtime が resume を諦めた needs_session の decide の ask（resume workspace は作らない）
