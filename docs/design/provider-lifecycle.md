...
3. maintainer の監視手順（AGENTS.md。[ADR-0010](../adr/0010-maintainer-and-resident-supervisor.md) の T3 以降は plugin の skill `taskq-maintain`）に、未信頼の repository で `supervise` を始めてしまった場合の扱いを足す: 最初の承認より前に起動した session（最大 `--parallel` 件）はすべて dialog で止まるので、それぞれに `send-key down` + `enter` で応答する。承認後に起動した session には出ない
