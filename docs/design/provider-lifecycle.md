...
1. operator 手順として文書化する（runtime 変更なし、推奨）: ある repository で初めて `supervise` を流す前に、その repository の root で `claude` を一度起動して dialog を承認する（または `~/.claude.json` の `projects[<root>].hasTrustDialogAccepted` が真であることを確認する）。使い捨て repository のスモーク（journal 010 の手順）も root を先に信頼する。`plugins/claude-taskq/skills/taskq-run/SKILL.md` の「every run's worktree is a directory Claude Code has never seen」という本文と、frontmatter `description` の「answer the trust prompt a real session stops at in each run's workspace」は誤りなので、この条件に書き換える
...
3. SV の監視手順（AGENTS.md）に、未信頼の repository で `supervise` を始めてしまった場合の扱いを足す: 最初の承認より前に起動した session（最大 `--parallel` 件）はすべて dialog で止まるので、それぞれに `send-key down` + `enter` で応答する。承認後に起動した session には出ない
