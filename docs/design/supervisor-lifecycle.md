...
結果は`IntegrationOutcome`: `{"outcome":"integrated","task":…,"run":…,"verification_skipped":…}`（`verification_skipped`はステップ5の再検証をskipしたかどうか。`run_integrated`イベントのpayloadにも同じ項目が入る）、`{"outcome":"needs_session","run":…,"main":…,"reason":…}`、`{"outcome":"failed","run":…,"reason":…}`、`--next`で対象がなければ`{"outcome":"no_run_awaiting"}`。
