...
- resume: `needs_session`のrunを`claude --resume <run-id>`で開き直すworkspaceは、workerと同じ`run_workspace_name`の名前でdescriptionを`run <run-id> resume`にする（当面はmaintainerが`dagq-session` skillに従って開く。goal 8の自動resumeでruntimeが開くときも同じ関数を使う）
