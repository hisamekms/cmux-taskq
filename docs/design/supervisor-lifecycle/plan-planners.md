...
- **test**: `tests/lifecycle_plan.rs`がfakeのcmuxで`plan`の記録・title・env・見た目・閉じたplannerの片付け・workspaceを作れなかったとき、runtimeが立てるplanner、wrapperと状態の判定を、`tests/e2e.rs`の`plan_opens_planners_side_by_side_that_submit_go_idle_and_exit`が実cmuxとstubのClaudeで、2つのplannerを同時に開き、それぞれがproposalをsubmitしてidleになり、`/exit`で終わるまでを確かめる。
