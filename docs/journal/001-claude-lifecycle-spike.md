...
# 001: Claude Code lifecycle spike

## Goal

cmuxとClaude Codeの接続方法をRust実装前に確かめる。使い捨てrepositoryで起動から成果とreceiptの回収まで1件を人が観察しながら通せること。

## Log

このタスクはジャーナル導入前に完了したため、経過は残っていない。以下は再現手順と観測した事実。
...
- [design/supervisor-lifecycle.md](../design/supervisor-lifecycle.md): receiptとセッション終了の分離、wrapperによる終了コード保存
- [design/provider-lifecycle.md](../design/provider-lifecycle.md): TTY継承、`--session-id`、位置引数でのprompt
- [plans/current.md](../plans/current.md): 入力待ちと異常系をステップ3・4へ引き継ぎ
