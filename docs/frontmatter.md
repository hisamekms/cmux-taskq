...
| --- | --- | --- |
| `adr` | `proposed`, `accepted`, `rejected`, `superseded`, `deprecated` | `accepted_on`, `superseded_by`, `superseded_on`, `deprecated_on`, `supersedes` (see [ADR fields](#adr-fields)) |
...
The rules follow [ADR-0042](adr/0042-adr-is-superseded-whole-and-deprecation-date-is-deprecated-on.md). Only an `accepted` ADR is a current decision, and every decision in its body is in force.
...
- **Append-only.** An ADR is append-only. Later, only `status`, `accepted_on`, `superseded_by`, `superseded_on`, `deprecated_on`, and the banner line may change, and these changes need no new ADR. `supersedes` is not among them: it is written together with the body (the reason for the replacement and the carried-over decisions) when the replacing ADR is written. Any other change to the body (adding, changing, or removing a decision) is made by a new ADR that replaces the old one as a whole. `updated` stays the last content change and does not move when only these fields change.
