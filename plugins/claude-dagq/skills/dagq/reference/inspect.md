...
| `"$DAGQ" stats` | Where time goes, from run events: `runs` (latest 50 finished; `--full` all) with seconds of `work` (claim→receipt), `validate` (receipt→validation), `wait_to_land` (validation→integrated), `startup`, and counts `resumes`, `needs_session`, `failed`, `review_verdict`; `goals` and `overall` with `{count, total, median}` per interval; `alerts` (`{kind, task_id, run_id, value, threshold}`: `awaiting_integration` > 15 min, 3rd `needs_session`, `ask_unanswered` > 60 min, `task_failed` twice, `work_over_median` > 2× the goal median, `idle_slots` with ready tasks all blocked, `claim_held` free slots while the supervisor holds new claims (load average above `supervise --max-load`; replaces `idle_slots`), `claim_deferred` free slots while tasks wait because their files meet a run in flight on a conflict hotspot (ADR-0069; replaces `idle_slots`), `backend_failures` 2 or more failed cmux calls in the window); `claim_holds` (`{count, secs, by_reason, held}`: the claim holds (`claim_held` / `claim_resumed` queue events) started in the window and the hold now); `claim_deferrals` (`{count, secs, by_end, by_file, deferred}`: tasks passed over at claim because their files meet a run in flight on a conflict hotspot, how each deferral ended (`cleared` / `expired` / `not_candidate` / `claimed` / `open`), and the tasks deferred now, which `status` also lists as `claim_deferrals`); `backend_failures` (`{count, by_op, max_load_avg, max_slots}`: `backend_call_failed` events in the window, per op, the highest 1-minute load and slots held); `next_cursor` — pass it to `--since` for only runs finished later. `kinds` groups the runs by their task's `kind` like `goals` (`kind: null` last). Each run also has `title`, `kind`, `claimed_at` / `validated_at` / `landed_at`, `integrate_attempts`, `deferrals` (per code), `conflict_files`, `broken_by` (the landings on main its landing was deferred after), `broke_runs`, and `resume_attempts` (`{reason, secs, resolved}`); `goals` and `overall` add `resume_outcomes` (per reason, with `resolved_percent`). `--since` / `--until` take an event id, `@<unix seconds>` or an RFC 3339 time. `--goal ID` narrows to one goal |
...
A queue with many tasks cannot be compared by reading every task in full. Take candidates from `search` and `related`, read only those in full (`show ID --full`), and judge only them (ADR-0046 decisions 6 and 7):
