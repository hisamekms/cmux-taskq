...
The runtime resumes a `needs_session` run itself (a landing conflict or failed verification, missing evidence, or a `send_back` answer; a `revise`, and a conflict with main found before a passed run's `/exit` (`conflict_precheck`), go to the still-open session instead). While it does, `status` shows `next: resuming (runtime)`: do nothing for that run. Never open a resume workspace or type into it; the one exception is a dialog the resumed session stops at, which you answer as in section 1 (find the workspace, described `run <run-id> resume`, in `cmux workspace list`). `${CLAUDE_PLUGIN_ROOT}/skills/dagq-session/reference/resume.md` has what the resume sends and when it ends.
