...
- `attention`: each with `run_id`, `task_id`, `status`, `kind` (the run event that brought it there), `last_error` and a fixed `next`:
  ...
  - `review by hand`: `awaiting_integration` after `review_failed` (the headless review failed); the person reviews it and has it integrated (the `dagq-recover` skill, section 6).
