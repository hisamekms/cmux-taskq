...
2. `attention`: what waits for the user or you now. Each entry has `run_id`, `task_id`, `status`, `kind`, `last_error` and a fixed `next`. Route it:
   ...
   - `resume session` (`needs_session`), `inspect and close workspace` (`failed`), `send /exit` (exit request timed out), `answer the prompt in workspace <id>` (`kind` `prompt_waiting`: a worker stopped at a dialog): the `dagq-session` skill.
