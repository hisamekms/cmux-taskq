---
name: dagq-maintain
description: Maintain a dagq queue: start its runtime with up (a launchd-resident supervisor that runs dependency-ready tasks in parallel, plus the maintainer's own session), read status to tell a registered, alive supervisor from a stale one, watch each run with show, answer the trust and permission prompts a run's session stops at, review a validated run and land it on main with integrate, resume a run left in needs_session, close the workspace of a failed or interrupted run, report a receipt's follow_ups, and stop the runtime with down. Use when the user asks to start, run, or supervise queued tasks, to check whether a run finished, to integrate or land a finished task, to deal with a run in needs_session, to clean up a failed run, or to shut the supervisor down.
---

# dagq: run tasks and land them on main

Prerequisite: resolve the launcher as in the `dagq` skill (`DAGQ="${CLAUDE_PLUGIN_ROOT}/bin/dagq"`, `"$DAGQ" --resolve`), including how it reports a missing binary and a `plugin_version` / `binary_version` mismatch. If it is not in context, read `${CLAUDE_PLUGIN_ROOT}/skills/dagq/SKILL.md` first. Never touch the queue database directly.

## 0. Roles

Three roles share one queue:

- **supervisor**: the resident `dagq supervise` process. It claims dependency-ready tasks up to `--parallel`, creates a Git worktree and a cmux workspace per run, validates each receipt, and closes the workspace of an accepted run. It is a process, not a session, and `up` keeps it resident.
- **maintainer**: the Claude Code session following this skill. It registers work (the `dagq` skill), starts and stops the runtime, watches the runs, answers their prompts, reviews what they produced, and lands it with `integrate`. It is the only role that pushes `main`.
- **worker**: the Claude session of one run, inside that run's worktree. It commits to `dagq/<run-id>` and writes the receipt; it never merges, pushes, or closes its workspace.

## 1. Start the runtime

```sh
"$DAGQ" up --plugin-dir "$CLAUDE_PLUGIN_ROOT"
"$DAGQ" up --parallel 4 --plugin-dir "$CLAUDE_PLUGIN_ROOT"    # --parallel defaults to 4
...
- `maintainer`: `{"outcome": "created" | "reused" | "skipped", "workspace_id", "name"}`. **`skipped` is the normal answer when you call `up` from inside the maintainer session**: that session is marked with `DAGQ_ROLE=maintainer` and `DAGQ_QUEUE`, so `up` does not open a second one. It is not an error, and the supervisor was still started or reused.
...
`candidates` must list the task; otherwise make it `ready` or complete its dependencies first (the `dagq` skill). `status` shows `supervisors`, one entry per process that owns runs, and the unfinished runs with their leases. Read each supervisor entry as:
...
`status` also returns `attention` and `cursor`. `attention` lists what waits for you or the user now, each with `run_id`, `task_id`, `status`, `kind` (the run event that brought it there), `last_error` and a fixed `next`: `review and integrate` (`awaiting_integration`), `resume session` (`needs_session`), `inspect and close workspace` (`failed`), `send /exit` (a `running` run whose exit request timed out), or `restart supervisor` (`kind` `supervisor_stale` with its `pid`, or `supervisor_stopped` when nothing is registered). `cursor` is the newest event id. To be woken instead of polling, run `"$DAGQ" watch --after <cursor>` in the background: it blocks (default `--timeout 600`, `--interval 2`) until an attention event arrives or the supervisors' registrations or `alive`/`stale` change, then returns `events`, `supervisors_changed`, `supervisors` and the next `cursor`; on timeout `events` is empty and the cursor is unchanged, so start it again with the same cursor. `"$DAGQ" events --after <cursor>` reads the same attention events without waiting (`--all` for every kind, `--limit N`). Both only read the queue; landing stays your call with `integrate`.
...
`show ID` prints only the latest run, the latest 10 events (`--events N` for more) with the gist of their payload, and long texts cut to 300 characters ending in `…` with `truncated: true`; use `show ID --full` when you need `run_dir`, earlier runs, or an event's whole payload such as a receipt.
...
- `integrating`: an `integrate` process holds the queue's single integration slot for this run (its lease shows in `status`). Wait for it; if its process died (`doctor` shows `lease_stale: true`; `doctor --full` shows its PID dead), the `dagq-recover` skill returns the run to `awaiting_integration`.
...
- `failed`: rejected or exited nonzero; `last_error` says why (missing or inconsistent receipt, dirty worktree, verification command failure). Workspace and worktree are kept for inspection; close the workspace yourself afterwards (below). Retry with `"$DAGQ" ready ID` after fixing the cause; the resident supervisor makes a new run.
- unfinished with `last_error` set and no lease in `status` (a `runtime_error` event with `lease_released: true`): the supervisor gave this run up (wrapper heartbeat lost, provisioning or validation error) and kept serving the others; use the `dagq-recover` skill. After a provisioning failure the supervisor stops claiming, finishes its active runs and exits nonzero; launchd restarts it, so fix the cause (cmux, Git) and check `status`.
...
`workspace_id` is the run's `workspace_id` in `"$DAGQ" show ID` (`doctor` lists only unfinished runs, so a `failed` run is not there). The session in it has already exited (both states require that), so only the shell is left; closing the workspace discards its screen, so read it first if `last_error` is not enough. The worktree, branch, and run directory stay on disk for the user's manual cleanup, as the `dagq-recover` skill describes.

One run's failure never changes another run: each is judged, closed and released on its own. Completion is decided only by these states. A Stop hook, an idle session, or a `receipt.json` appearing in `run_dir` is not success: the receipt's claims are cross-checked by the supervisor, and `validation_finished` in `events` records the verdict and reason. If a run stays `running` after `exit_request_timed_out` (attention `send /exit`), something in the session, usually one of Claude Code's own dialogs, held the `/exit` back. The supervisor keeps the run and its lease and keeps watching, so do not `recover` it or `ready` the task again: read the workspace screen, deal with the dialog (ask the user when it needs their judgment), and send `/exit` in that task's workspace. Once the session exits, the run goes on to `validating` as usual. The supervisor never resends `/exit` itself. Until the session exits, the run holds its slot and any drain (`down --wait`, `up` replacing a supervisor of another version) waits for it, so send `/exit` before draining.

## 5. Review and land

When the run is `awaiting_integration` (or back from a session as `needs_session`), review it outside this session's context. Do not read the full `git diff` in this session: it is the one unbounded input a maintainer gets. Instead:

```sh
"$DAGQ" review ID
```

writes `<run_dir>/review.md` (temporary file, then rename) and returns only `{"run_id", "task_id", "path", "base", "head", "files_changed", "insertions", "deletions"}`. The file holds the task's title, description, acceptance and verification commands, the goal's acceptance and constraints when the task has a goal, the receipt's summary, tests, e2e, subagent_review and follow_ups, `git log --oneline <base>..<head>`, `git diff --stat <base>...<head>`, and finally the full `git diff <base>...<head>` (`base` is the run's base commit, or the current `main` once a session rebased `head` onto it; `head` is the receipt's commit). `review` refuses a task with no run `awaiting_integration` or `needs_session`.

Hand `path` to a subagent (the Agent tool) and ask it to read the file and check the diff against the task's acceptance, the goal's constraints and the receipt's claims, and to answer with a verdict and findings only, not the diff. Take only that conclusion back; tell the user the branch, `head`, the diffstat numbers and the subagent's verdict. The `verify-N.log` files in `run_dir` (`show ID --full`) hold the supervisor's own verification output; `integrate` writes its own re-validation logs there as `integrate-verify-N.log`, but only when it reruns the verification commands (see below). A run can still hold `integrate-verify-N.log` files from an earlier `integrate` attempt (one that ended in `needs_session`, say) even though its final landing skipped the rerun; tell them apart by the run's events (`show ID --full`), not by the files: after the last `integration_rebased`, either `verification_command` events with `phase: "integration"` (a rerun, which wrote the current logs) or `integration_verification_skipped` (no rerun, so any `integrate-verify-N.log` is older). A run that changed the runtime itself is also worth a `cargo test --locked --test e2e -- --ignored` in its worktree before landing, when the repository asks for it. If the receipt has `follow_ups` (work the worker found outside its task, as `{title, description}`; also in the `receipt` of the `validation_finished` event in `show ID --full`), report them to the user before `integrate` and again with the outcome after it, so they are registered with the `dagq` skill (`add --goal ID` on the task's goal, or a plain `add` when the task has none) rather than lost with the run. Never merge, rebase, or fast-forward the branch yourself: landing is the runtime's job and it keeps `main` linear with one squash commit per task. Only land when the user has approved the run.
...
`integrate` takes the single integration slot, rebases the run's worktree onto the current `main`, re-validates it (the receipt must name the worktree head, the rebased head must sit on `main` with a clean tree, and the task's verification commands are rerun unless the rebase was a no-op on the head validation already verified; see below), squashes the rebased tree into one commit on `main` (title, receipt summary, trailers `Dagq-Task: ID` and `Dagq-Run: RUN_ID`), and removes the worktree and branch; the run's history stays under `refs/dagq/runs/<run-id>`. When the rebase is a no-op and the worktree head is still the run's `result_commit` (the commit the supervisor's validation already ran these commands on), `integrate` skips the rerun and writes no `integrate-verify-N.log`; a head a session rewrote after `needs_session` is always rerun, even if it already sits on `main`. A skipped rerun shows as an `integration_verification_skipped` event (`main`, `head`, `reason`) and as `verification_skipped: true` in `integrate`'s output and in the `run_integrated` event payload; a rerun shows as `verification_command` events with `phase: "integration"`. Read the `outcome`:
...
- `needs_session`: nothing reached `main`; `reason` names the conflicting files (the rebase was aborted and the worktree is back on `result_commit`) or the failed verification command, whose output is in `integrate-verify-N.log` in `run_dir` (`show ID --full`; the rebased tree is left in the worktree). Continue with step 6.
...
An error (exit status 1) before `main` moved puts the run back to its previous state with the message in `last_error`; a `main` checkout with uncommitted changes that overlap the landing is a common cause. Fix it and run `integrate` again. The landing happens in the current directory's repository; pass `--repo PATH` only when using `DAGQ_DB` from outside it.
