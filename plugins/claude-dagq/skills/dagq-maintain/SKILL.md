...
`integrate` takes the single integration slot, rebases the run's worktree onto the current `main`, re-validates it (the receipt must name the worktree head, the rebased head must sit on `main` with a clean tree, and the task's verification commands are rerun), squashes the rebased tree into one commit on `main` (title, receipt summary, trailers `Dagq-Task: ID` and `Dagq-Run: RUN_ID`), and removes the worktree and branch; the run's history stays under `refs/dagq/runs/<run-id>`. Read the `outcome`:
