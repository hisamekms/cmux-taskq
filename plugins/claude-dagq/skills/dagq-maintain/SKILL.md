...
- `integrating`: an `integrate` process holds the queue's single integration slot for this run (its lease shows in `status`). Wait for it; if its process died (`doctor` shows the lease stale and its PID dead), the `dagq-recover` skill returns the run to `awaiting_integration`.
...
A `needs_session` run is fixed by a Claude session in its own worktree, not by this session and not by hand. Take `worktree_path`, the run `id`, and `last_error` from `"$DAGQ" show ID`, then open a workspace that resumes the run's session (its session ID is the run ID):
