...
- unfinished with `last_error` set and no lease in `status` (a `runtime_error` event with `lease_released: true`): the supervisor gave this run up (wrapper heartbeat lost, exit request timed out, provisioning or validation error) and kept serving the others; use the `dagq-recover` skill. After a provisioning failure the supervisor stops claiming, finishes its active runs and exits nonzero; launchd restarts it, so fix the cause (cmux, Git) and check `status`.
