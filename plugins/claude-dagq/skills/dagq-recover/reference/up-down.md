...
`up` does not replace a supervisor that is alive but no longer heartbeating (it starts a new one beside it and `status` shows the old row as `stale`; the person stops it with `down --force`), and it does not notice a rebuild that did not bump the version, since `binary_version` is `CARGO_PKG_VERSION`: then `down --wait`, replace the binary, and `up`.
