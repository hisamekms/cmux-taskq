...
- `inbox` and `planner`: `{"outcome": "created" | "reused" | "skipped", "workspace_id", "name"}`. `up` finds each workspace by the UUID it recorded in the queue, never by its title: `reused` while cmux still lists it (renaming it changes nothing), `created` when it is gone. `skipped` is the normal answer for the session `up` is called from: the workspace carries `DAGQ_ROLE=inbox` / `planner` and `DAGQ_QUEUE` in its own environment (`cmux workspace env <id> --json`), so `up` does not open a second one. `down` closes neither.
- `up` pins the inbox's and the planner's workspaces (ADR-0031), and cmux refuses `cmux workspace close` on a pinned workspace (`Error: protected: ...`). To close one by hand, unpin it first: `cmux workspace-action --action unpin --workspace <id>`, then `cmux workspace close <id>`. The unpin succeeds on an unpinned workspace too. The supervisor's and the workers' workspaces are not pinned.
- A workspace the queue recorded for a role `up` no longer opens (a retired resident session, or the resident planner ADR-0041 decision 6 dropped) is forgotten by `up`, which leaves the workspace itself open: the person closes it with `cmux workspace close <id>`.
...
After the person has swapped the `dagq` binary, a plain `up` is the whole update: it unloads the LaunchAgent, signals the old supervisor, waits (with no timeout) for it to stop claiming and finish the runs it holds, closes its workspace if it ran in one, and starts a supervisor of the new version. Tell the person before replacing the binary. The result is `"outcome": "restarted"` with `previous_version`, `version` and `replaced`. The wait is the length of the runs in flight, so tell the person what it waits for rather than killing it. `up --no-wait` refuses instead whenever a run is in flight (it names the count and the run IDs and changes nothing); with nothing in flight it replaces the supervisor but gives up after 30 seconds if that supervisor does not stop.

Opening a queue never migrates it (ADR-0045). If the new binary's `up` (or any command) stops with "run `dagq migrate`", the new binary has migrations the queue lacks. `dagq migrate --check` lists them with `compatible`. Compatible ones apply with `dagq migrate` while everything runs. A breaking one is refused while a supervisor, run or wrapper uses the queue: `down --wait` with the old binary first (put it back if you already replaced it), then replace the binary, `dagq migrate` (it copies the database to `backups/` first), and `up`. Tell the person before the drain. A command that says `unsupported queue schema version ... install a newer dagq` means the queue was migrated past this binary: use the newer fixed binary.

`up` does not replace a supervisor that is alive but no longer heartbeating (it starts a new one beside it and `status` shows the old row as `stale`; the person stops it with `down --force`). `binary_version` is the build identifier (`X.Y.Z` for a release, `X.Y.Z-dev+<commit>[.dirty]` for a build of main), so a rebuild from another commit is replaced like a new release. An older fixed binary still names itself by `CARGO_PKG_VERSION` alone and misses a rebuild that did not bump the version: then `down --wait`, replace the binary, and `up`.
