...
- A workspace the queue recorded for a role `up` no longer opens (the resident session ADR-0024 retired) is forgotten by `up`, which leaves the workspace itself open: the person closes it with `cmux workspace close <id>`.
