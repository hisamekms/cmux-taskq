...
## approve_landing answers without a command

A run with a `concern` stays `awaiting_integration` while its `approve_landing` ask is open; `status` keeps showing it as `review and integrate`, and the open ask on the same run is what says the review is done. Asking again for the same run returns the same ask (`created: false`) while it is open; once it is answered and closed, only the run's `send_back` / `cancel` note (below) keeps a later review from opening a new one.

`land` goes through `integrate` like a pass. `send_back` and `cancel` have no command yet: the runtime resumes a run's session only after a failed `integrate` (`needs_session`), and `cancel ID` is refused while the task owns an unfinished run, which includes `awaiting_integration`. Until the runtime has one, record the answer as a note on the run (`"$DAGQ" note --run <run_id> --kind send_back|cancel --text "<the answer>"`), close the ask, and report the run to the person as waiting for them: the note keeps later reviews from landing it, and the worktree and branch stay as they are. Never edit the run's worktree, `recover` it or open a session for it to get around this.
