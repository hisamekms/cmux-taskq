...
-- The supervisor's check of the programs dagq.toml's [run.env] names
-- (ADR-0049 decision 9) records `run_env_program_missing` and
-- `run_env_program_found` on the queue itself, on no task, goal or run.
-- SQLite cannot change a CHECK, so run_events is rebuilt like 0030 with the
-- two kinds added; ids are preserved, and the triggers on it, and the one on
-- tasks that reads it, are created again. Breaking: an older binary cannot
-- write a queue whose events it would reject.
DROP TRIGGER search_task_moved;
CREATE TABLE run_events_v32 (
...
INSERT INTO run_events_v32 (id, task_id, goal_id, run_id, kind, payload, created_at)
SELECT id, task_id, goal_id, run_id, kind, payload, created_at FROM run_events;
DROP TABLE run_events;
ALTER TABLE run_events_v32 RENAME TO run_events;
