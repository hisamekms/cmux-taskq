...
- test ファイルの行数: `tests/*.rs` と `tests/common/*.rs` はどれも 3,000 行以下に保つ（goal 47）。末尾に test を足す形の大きなファイルは別々の task の追記が同じ場所で衝突するため。`scripts/check-test-file-lines.sh` が超えたファイルを名前と行数つきで出して exit 1 にし、CI も実行する。超えそうなら、test を足す前に機能ごとのファイル（`tests/runtime_*.rs`・`tests/lifecycle_*.rs` のように）へ分け、複数のファイルで使う helper は `tests/common` に寄せる。`src/` はまだ対象外
