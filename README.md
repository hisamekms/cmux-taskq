...
| `/claude-dagq:dagq-inbox` | The inbox session: `status --role inbox`, `watch --role inbox` in the background, show each open ask with its options to the person and write their answer with `answer`; it decides nothing itself, and the supervisor applies an `approve_landing` answer |
...
| `/claude-dagq:dagq-land` | Read the supervisor's review verdict; only for a run whose headless review failed, write `review.md` with `review ID`, have a subagent review it and return only a verdict (`pass` / `concern`), then `integrate` on a pass without asking (`integrate` pushes `main` and registers a receipt's `follow_ups` as draft tasks); on a concern register an `approve_landing` ask (`land` / `send_back` / `cancel`), whose answer the supervisor applies |
