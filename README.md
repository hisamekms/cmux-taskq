...
| `up [--parallel N] [--in-cmux] [--plugin-dir PATH] [--repo PATH] [--cmux EXE] [--claude EXE]` | Start the queue's runtime, idempotently: prune supervisor registrations whose process is gone, keep the supervisor resident as a launchd LaunchAgent (started only when no live one is registered), and open the maintainer's Claude Code session in the cmux workspace `taskq <repo> maintainer` (skipped inside that session, reused if open). `--in-cmux` runs the supervisor in the cmux workspace `taskq <repo> supervisor` instead, with no launchd and no automatic restart. Reports what needs attention: unfinished runs, runs awaiting integration, runs that need a session |
...
| `status` | List every registered supervisor (PID, liveness, heartbeat age, `mode` and `workspace_id`, `parallel`, the runs it holds), any other lease holder such as an `integrate` process, and the unfinished runs with their leases |
...
`status` and `doctor` show which mode is running: each entry under `supervisors` carries `mode` (`launchd`, `in_cmux`, or `null` for one you started by hand) and, for `in_cmux`, the `workspace_id` it runs in. `up` records this on the registration when the supervisor it started comes up, so it lives and dies with that registration.
