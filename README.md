...
The runtime never closes the cmux workspace of a failed or interrupted run. Once you are done inspecting it, close it yourself with `cmux workspace close <workspace_id>`; the workspace ID is in the `show` and `doctor` output for that run. If you instead resolved the run in a `claude --resume <run-id>` session (see below), exit that session with `/exit` before running `integrate ID`, because landing removes the worktree the session is working in.
...
| `/claude-taskq:taskq-run` | Launch `supervise --parallel N` in a dedicated cmux workspace whose `--cwd` is the repository, watch runs with `show` and `status`, judge completion from the run state and receipt rather than a Stop hook, review and land with `integrate`, resume a `needs_session` run |
