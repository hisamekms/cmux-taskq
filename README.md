...
| `up [--parallel N] [--in-cmux] [--no-wait] [--plugin-dir PATH] [--repo PATH] [--cmux EXE] [--claude EXE]` | Start the queue's runtime, idempotently: prune supervisor registrations whose process is gone, keep the supervisor resident as a launchd LaunchAgent (started only when no live one of this binary's version is registered), and open the inbox's and the planner's Claude Code sessions in the cmux workspaces `[<repo>]inbox` and `[<repo>]planner` (each skipped inside that session itself, reused if open). A live supervisor of another version is drained and replaced (`--no-wait` refuses that instead whenever a run is in flight). `--in-cmux` runs the supervisor in the cmux workspace `[<repo>]supervisor` instead, with no launchd and no automatic restart. Reports what needs attention: unfinished runs, runs awaiting integration, runs that need a session |
